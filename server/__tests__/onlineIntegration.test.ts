...
describe('E2E Online Multiplayer Integration Test Suite', () => {
  ...
  beforeAll(async () => {
    ...
    ioServer.on('connection', (socket) => {
      ...
      socket.on('GAME_ACTION', (data: { roomCode: string; action: any }, callback) => {
        const room = rooms.get(data.roomCode?.trim().toUpperCase());
        if (!room || !room.gameState) {
          if (typeof callback === 'function') callback({ success: false, message: 'Active game not found.' });
          return;
        }
        const activePlayer = room.gameState.players[room.gameState.currentTurnIndex];
        const senderSessionId = socket.data.sessionPlayerId;

        if (activePlayer.id !== senderSessionId) {
  ...
  it('Case 7: Should handle room code whitespace and lowercase trimming seamlessly', async () => {
    const untrimmedCode = `  ${testRoomCode.toLowerCase()}  `;

    const joinRes: any = await new Promise((resolve) => {
      client1.emit('JOIN_ROOM', { roomCode: untrimmedCode, playerName: 'Alice', sessionPlayerId: sessionUser1 }, resolve);
    });

    expect(joinRes.success).toBe(true);
    expect(joinRes.roomCode).toBe(testRoomCode);
  });

  it('Case 8: Should reject non-existent room code with structured error message', async () => {
    const joinRes: any = await new Promise((resolve) => {
      client1.emit('JOIN_ROOM', { roomCode: 'INVALID999', playerName: 'Alice', sessionPlayerId: sessionUser1 }, resolve);
    });
  ...
  it('Case 9: Should enforce turn validation and reject non-turn player GAME_ACTION', async () => {
    const room = rooms.get(testRoomCode);
    room.gameState = {
      currentTurnIndex: 0,
      players: [{ id: sessionUser1, name: 'Alice' }, { id: sessionUser2, name: 'Bob' }]
    };

    // Client 3 (sessionUser3) attempts to send action when it's sessionUser1's turn
    const client3Test = await createTestClient(sessionUser3);
    const actionRes: any = await new Promise((resolve) => {
      client3Test.emit('GAME_ACTION', { roomCode: testRoomCode, action: { type: 'TAKE_2_SAME' } }, resolve);
    });

    expect(actionRes.success).toBe(false);
    expect(actionRes.message).toBe('Not your turn.');
    client3Test.disconnect();
  });

  it('Case 10: Should accept valid GAME_ACTION when submitted by active turn player', async () => {
    const actionRes: any = await new Promise((resolve) => {
      client1.emit('GAME_ACTION', { roomCode: testRoomCode, action: { type: 'TAKE_3_DISTINCT' } }, resolve);
    });
