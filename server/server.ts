...
io.on('connection', (socket: Socket) => {
  ...
  console.log(`Socket connected: ${socket.id} (Session: ${sessionPlayerId})`);
  ...
  socket.on('CREATE_ROOM', (data: { playerName: string; config: GameConfig; sessionPlayerId?: string }, callback) => {
    ...
    const room: RoomInfo = {
      ...
      hostId: playerId,
      players: [
        { id: playerId, name: (data.playerName || 'Host Player').trim(), isBot: false, isReady: true }
      ],
      config: data.config,
      isStarted: false,
      gameState: null
  ...
  socket.on('JOIN_ROOM', (data: { roomCode: string; playerName: string; sessionPlayerId?: string }, callback) => {
    ...
    // Check if player is re-joining an existing room
    ...
    room.players.push({ id: playerId, name: (data.playerName || `Player ${room.players.length + 1}`).trim(), isBot: false, isReady: true });
  ...
  socket.on('ADD_BOT', (data: { roomCode: string; difficulty: 'easy' | 'medium' | 'hard' }) => {
    ...
    io.to(`room_${room.roomCode}`).emit('ROOM_UPDATED', room);
  });
  ...
  // 5. SUBMIT ACTION
  socket.on('SUBMIT_ACTION', async (data: { roomCode: string; action: GameAction; sessionPlayerId?: string }, callback) => {
    const roomCode = data.roomCode?.trim().toUpperCase();
    const room = rooms.get(roomCode);
    ...
    const mutex = getRoomMutex(roomCode);
    await mutex.runExclusive(() => {
      if (!room.gameState) return;
  ...
  socket.on('SEND_CHAT', (data: { roomCode: string; message: string; senderName: string }) => {
    const roomCode = data.roomCode?.trim().toUpperCase();
    io.to(`room_${roomCode}`).emit('CHAT_MESSAGE', {
...
function startServer(port: number) {
  server.listen(port, () => {
    console.log(`✅ Splendor Server running on http://127.0.0.1:${port}`);
  }).on('error', (err: any) => {
    if (err.code === 'EADDRINUSE') {
      console.warn(`Port ${port} in use. Attempting next port ${port + 1}...`);
      startServer(port + 1);
    } else {
