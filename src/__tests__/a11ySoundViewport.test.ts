...
describe('Accessibility, Audio, Haptics & Viewport Integration Test Suite', () => {
  ...
  it('Test 7: SoundManager mute state toggles without throwing errors', () => {
    const initialMute = soundManager.getMuted();
    const toggled = soundManager.toggleMute();

    expect(toggled).toBe(!initialMute);
    expect(soundManager.getMuted()).toBe(toggled);

    // Restore initial mute state
    soundManager.toggleMute();
  });

  it('Test 8: Window resize handler toggles is-resizing CSS class safety', () => {
