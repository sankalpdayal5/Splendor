...
describe('Game Engine Rule Invariants & State Machine Test Suite', () => {
  ...
  it('Test 11: Noble Tile Uniqueness Invariant — Claimed nobles are permanently removed from board pool', () => {
    let state = createInitialState(baseConfig);
    const nobleCountInitial = state.nobles.length;

    state.nobles = [
      { id: 'n1', name: 'Noble 1', prestigePoints: 3, reqs: { emerald: 1, diamond: 0, sapphire: 0, ruby: 0, onyx: 0 } } as any,
      { id: 'n2', name: 'Noble 2', prestigePoints: 3, reqs: { diamond: 3, emerald: 0, sapphire: 0, ruby: 0, onyx: 0 } } as any
    ];

    state.tierGrid.tier1[0] = { id: 'c1', tier: 1, gemBonus: 'emerald', prestigePoints: 0, cost: { ruby: 0 } } as any;

    state = applyAction(state, { type: 'BUY_GRID', tier: 1, slotIdx: 0 });

    expect(state.players[0].nobles.length).toBe(1);
    expect(state.players[0].nobles[0].id).toBe('n1');
    expect(state.nobles.length).toBe(1);
    expect(state.nobles[0].id).toBe('n2'); // n1 removed permanently from board
  });

  it('Test 12: Complete Tier Deck Depletion — Empty deck leaves null grid slot and prevents invalid buys', () => {
    let state = createInitialState(baseConfig);
    state.tierDecks.tier1 = []; // Deplete deck
    state.tierGrid.tier1[0] = { id: 'last_c1', tier: 1, gemBonus: 'ruby', prestigePoints: 0, cost: { ruby: 0 } } as any;

    state = applyAction(state, { type: 'BUY_GRID', tier: 1, slotIdx: 0 });

    expect(state.tierGrid.tier1[0]).toBeNull(); // Grid slot remains null when deck empty

    // Buying null slot is rejected
    const invalidBuy = validateAction(state, { type: 'BUY_GRID', tier: 1, slotIdx: 0 });
