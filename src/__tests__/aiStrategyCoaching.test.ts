...
describe('AI Engine, ISMCTS & Learn Mode Coaching Test Suite', () => {
  ...
  it('Test 9: Defensive Noble Denial — Hard AI applies bonus score when reserving card targeted by leading opponent', () => {
    let state = createInitialState(baseConfig);
    state.currentTurnIndex = 2; // Hard bot
    state.players[0].prestigePoints = 13; // Opponent leading
    state.players[0].cards = [
      { id: 'c1', tier: 1, gemBonus: 'emerald', prestigePoints: 0, cost: {} },
      { id: 'c2', tier: 1, gemBonus: 'emerald', prestigePoints: 0, cost: {} }
    ] as any;

    // Card on grid that enables noble visit for opponent
    state.tierGrid.tier2[0] = { id: 'noble_key', tier: 2, gemBonus: 'emerald', prestigePoints: 2, cost: { emerald: 1 } };
    state.nobles = [{ id: 'n1', prestigePoints: 3, reqs: { emerald: 3 } }] as any;

    const reserveScore = evaluateActionScore(state, { type: 'RESERVE_GRID', tier: 2, slotIdx: 0 }, 'hard');
    expect(reserveScore).toBeGreaterThan(50); // Receives defensive bonus weighting
  });

  it('Test 10: High-rollout ISMCTS simulation (250 rollouts) executes in <500ms', () => {
    const state = createInitialState(baseConfig);
    const moves = generateLegalActions(state, 0).slice(0, 2);

    const start = Date.now();
    const results = runISMCTSSimulation(state, moves, 250);
    const duration = Date.now() - start;
