...
import { applyAction, canAffordCard, cloneGameState } from './gameEngine.js';
...
export function runISMCTSSimulation(
  ...
  return candidateActions.map(action => {
    ...
    for (let r = 0; r < rolloutsPerAction; r++) {
      let simState = cloneGameState(state);
      try {
        ...
        for (let step = 0; step < 6; step++) {
          ...
          // Unbiased rollout selection across all legal move types (Buys, Reserves, Gem Takes)
          const rolloutPick = legal[Math.floor(Math.random() * legal.length)];
...
function generateProjectedPath(state: GameState, firstAction: GameAction): { turnOffset: number; actionDescription: string; projectedPP: number }[] {
  ...
  try {
    ...
    for (let step = 2; step <= 3; step++) {
      ...
      // Advance game turns through all other players until active player's next turn
      while (simState.currentTurnIndex !== activePlayerIndex && simState.phase !== 'FINISHED') {
        const botAction = selectBotAction(simState, 'hard');
        simState = applyAction(simState, botAction);
      }
      ...
      // Pick highest scoring follow-up action for active player
