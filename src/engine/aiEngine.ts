...
import {
  ...
  getTargetCardDeficits,
  getRecommendedDiscardTokens,
  formatResourceMap,
  isOpponentTargetingCard
} from './aiEvaluator.js';
...
export function generateLegalActions(state: GameState, playerIndex: number): GameAction[] {
  ...
  // Robust Fallback: Ensure legalActions is NEVER empty even in extreme end-game edge cases
  if (legalActions.length === 0) {
    const availableColors = GEM_COLORS.filter(col => (state.bank[col] || 0) > 0);
    const expectedCount = Math.min(3, availableColors.length);
    legalActions.push({ type: 'TAKE_3_DISTINCT', colors: availableColors.slice(0, expectedCount) });
  }
