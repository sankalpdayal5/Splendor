...
export const PlayerPanel: React.FC<PlayerPanelProps> = React.memo(({
  ...
  return (
    <div
      className={`glass-panel player-panel ${isActive ? 'active active-turn' : ''}`}
      style={{
        padding: '12px 14px',
        borderRadius: '12px',
        border: isActive ? `2px solid ${player.color}` : '1px solid rgba(255,255,255,0.1)',
        boxShadow: isActive ? `0 0 16px ${player.color}60, inset 0 0 10px ${player.color}20` : 'none',
    ...
    >
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '4px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          <span style={{ width: '10px', height: '10px', borderRadius: '50%', background: player.color, boxShadow: isActive ? `0 0 8px ${player.color}` : 'none' }} />
          ...
          {isActive && (
            <span
              style={{
                ...
                border: `1px solid ${player.color}90`,
                padding: '1px 6px',
                borderRadius: '4px',
                letterSpacing: '0.5px',
