...
import { Volume2, VolumeX, BookOpen, Sparkles, LogOut, User } from 'lucide-react';
...
interface NavbarProps {
  ...
  onExitGame?: () => void;
  onExitConfirmStateChange?: (isOpen: boolean) => void;
  onOpenProfile?: () => void;
...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  const toggleExitConfirm = (val: boolean) => {
    setShowExitConfirm(val);
    if (onExitConfirmStateChange) {
      onExitConfirmStateChange(val);
    }
  };
  ...
  return (
    <header className="navbar">
      ...
      <div className="nav-actions" style={{ display: 'flex', alignItems: 'center', gap: '6px', flexWrap: 'nowrap' }}>
        ...
        {onOpenProfile && (
          <button
            ...
            title="Open Merchant Profile"
          >
            <span style={{ fontSize: '16px' }}>{userProfile?.avatar || '👨‍💼'}</span>
            <span style={{ fontWeight: 700, maxWidth: '90px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
      ...
      {showExitConfirm && (
        <div className="modal-overlay" style={{ zIndex: 10000 }} onClick={() => toggleExitConfirm(false)}>
          <div
